...
async def run_tests():
    ...
    async with httpx.AsyncClient() as client:
        ...
        print(f"  ✅ Wrong shop rejected with {res.status_code}")
        ...
        print("\n🟢 Test 6 — Post-Simulation Anomaly Check")
        db = next(get_db())
        from app.models.anomaly import Anomaly
        final_anom = db.query(Anomaly).count()
        print(f"  Final Anomaly Count: {final_anom} (Baseline: {pre_anom})")
        assert final_anom == pre_anom, "Anomaly count changed during simulation-only cycle!"
        print("  ✅ Anomaly isolation confirmed")
